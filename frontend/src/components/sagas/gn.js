...
export function *get_news(data) {
  console.log('Hello from watchers', data);
...
export function *getNewsAsync(data) {
  ...
  const response = yield call(() => axios.get(`${server}api/news/`, { params: { page: data.data.page, news_on_page: data.data.news_on_page } }));
